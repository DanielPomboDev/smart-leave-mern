...
const MayorLeaveRequests = () => {
  ...
  const fetchLeaveRequests = useCallback(async () => {
    ...
    try {
      ...
      setFilteredRequests(response.leaveRequests || []);
    } catch (error) {
  ...
  if (loading) {
    return (
      <Layout title="Mayor Leave Requests">
                  ...
                  {filteredRequests.map((request) => (
                    <tr key={request.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm font-medium">
                        <button
                          onClick={() => navigate(`/mayor/leave-requests/${request.id}`)}
...
export default MayorLeaveRequests;